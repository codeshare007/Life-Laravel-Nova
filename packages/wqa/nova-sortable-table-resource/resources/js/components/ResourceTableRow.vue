<template>
    <tr :dusk="resource['id'].value + '-row'" @change="onChange">
        ...
        <!-- Fields -->
        <td v-for="field in resource.fields" :class="`td-${field.attribute}`">
            <component
                ...
                :edit-mode="editMode"
                :resource-row-id="resource['id'].value"
                :field="field"
                @change="onChange"
            />
        </td>
...
export default {
    ...
    created() {
        Nova.$on('onToggleFields_Resource'+this.resource.id.value, (args) => {
          this.onToggleFields(args);
        });
        Nova.$on('onChangeSelect_Resource'+this.resource.id.value, (args) => {
          this.onChange(args);
        });
        Nova.$on('onFieldInlineSave_Resource'+this.resource.id.value, (args) => {
          this.onFieldInlineSave(args);
        });
    },
    ...
    methods: {
      ...
      onFieldInlineSave(args) {
        let url = '/nova-vendor/wqa/nova-sortable-table-resource/'+this.resourceName+'/field-inline-save';
        let params = {
        ...
        return Nova.request()
          .post(url, params)
          .then(response => {
            Nova.bus.$emit('reloadOptionsAfterSave_Resource'+this.resource.id.value, response.data)
          })
      }
    },
}
