...
trait ResolvesFields
{
     ...
     * @return \Illuminate\Support\Collection
     */
    public function availableFieldCards(ResourceDetailRequest $request)
    {
        $cards = collect(array_values($this->fields($request)))
            ->whereInstanceOf(Card::class)->values();

        $default = Card::defaultNameFor($request->newResource());

        return $cards->when($cards->where('name', $default)->isEmpty(), function ($cards) use ($default) {
            return $cards->push((new Card($default)));
        })->all();
    }
