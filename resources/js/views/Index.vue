<template>
    <loading-view :loading="initialLoading" :dusk="resourceName + '-index-component'">
        ...
        <div class="flex" v-if="!viaResource">
            <!-- Search -->
            <div
            ...
            <div class="w-full flex items-center mb-6">
                <custom-index-toolbar v-if="!viaResource" :indexId="indexId" :class="{'mr-4': ! editMode}" />
        ...
        <div v-else>
            ...
            <custom-index-editmode :indexId="indexId" />
        </div>
...
export default {
    ...
    mounted() {
        this.watchForToggleEditMode();
        this.watchForModifiedFields();
        this.watchForEditModeSave();
        this.watchForReloadIndex();
    },
    ...
    destroyed() {
        ...
        // Unbind events
        Nova.$off('onFieldsUpdated');
        Nova.$off('onToggleEditMode');
        Nova.$off('triggerEditModeSaveChanges');
        Nova.$off('onReloadIndex');
    },
    ...
    methods: {
        ...
        watchForModifiedFields() {
            Nova.$on('onFieldsUpdated', (data) => {
                if (data.indexId === this.indexId) {
                    if (!this.editModeUpdates.hasOwnProperty(data.resourceId)) {
                        this.editModeUpdates[data.resourceId] = {};
                    }

                    let tempFormData = new FormData();
                    data.fields.forEach(fieldComponent => {
                        fieldComponent.field.fill(tempFormData);
                    });

                    for (var [fieldName, fieldValue] of tempFormData.entries()) {
                        this.editModeUpdates[data.resourceId][fieldName] = fieldValue;
                    }
                }
            });
        },

        watchForToggleEditMode() {
            Nova.$on('onToggleEditMode', (data) => {
                if (data.indexId === this.indexId) {
                    this.editMode = data.editModeOn;
                }
            });
        },

        watchForEditModeSave() {
            Nova.$on('triggerEditModeSaveChanges', (data) => {
                if (data.indexId === this.indexId) {
                    this.batchSave();
                }
            });
        ...
        batchSave() {
            if (Object.keys(this.editModeUpdates).length) {
                let url = '/nova-vendor/wqa/nova-sortable-table-resource/'+this.resourceName+'/batch-update';
                let params = {
                    resourceName: this.resourceName,
                    viaResourceId: this.viaResourceId,
                    updatedModels: this.editModeUpdates
                };

                return Nova.request().post(url, params).then(response => {
                    Nova.$emit('onEditModeSaved', {
                        indexId: this.indexId,
                    });

                    this.$toasted.show(
                        this.headingTitle + ' updated',
                        {type: 'success'}
                    );
                });

                this.editModeUpdates = {};
            } else {
                Nova.$emit('onEditModeSaved', {
                    indexId: this.indexId,
                });
    ...
    computed: {
        /**
         * Generate a unique id for each of the indexes
         */
        indexId() {
            if (this.viaResource) {
                return `${this.viaResource}-${this.viaRelationship}-${this.resourceName}`;
            } else {
        ...
        poly() {
          return this.isRelation && this.field ? this.field.poly : null
        },
        ...
        // relationFormFields() {
        //     var fields = JSON.parse(JSON.stringify(this.resources[0].fields));
