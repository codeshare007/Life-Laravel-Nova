...
namespace App\Http\Controllers\Api\v2_1\Recipe;
...
class RecipeController extends BaseQueryBuilderController
{
    /**
     * @var Recipe
     */
    ...
    public function __construct(
        ...
        parent::__construct($recipe, $request, $this->allowedIncludes);
    }

    /**
     * @OA\Get(
     *     path="/{lang}/v2.1/recipes",
     ...
     *         @OA\JsonContent(
     *             type="array",
     *             @OA\Items(ref="#/components/schemas/RecipeCollectionResource2.1")
     *         ),
     *     ),
     *     @OA\Response(
     *         response=400,
    ...
    {
        if (! Cache::has(ElementCacheKey::Recipe)) {
            Cache::forever(
                ElementCacheKey::Recipe,
        ...
        $data = Cache::get(ElementCacheKey::Recipe);

        return api_resource(ApiCollectionResource::Recipe)
            ->make($data);
    }
}
